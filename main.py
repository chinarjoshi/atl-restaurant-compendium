...
def build_atl_mag_restaurants(soup: BeautifulSoup) -> dict[str, str]:
...
def build_atl_eater_restaurants(soup: BeautifulSoup) -> dict[str, str]:
...
def build_midtown_restaurants(soup: BeautifulSoup) -> dict[str, str]:
...
data = Path("data")
website_metadata = [
    {"url": "https://www.atlantamagazine.com/50bestrestaurants/", name: "atl_mag.html"},
    {
...
with open(data / 'atl_mag.html') as fm,
        open(data / 'atl_eater.html') as fe,
        open(data / 'midtown.html') as fm:
